...
public class RemainDetailAPI {
	...
	public static final String REQUEST_URL  = "https://api.odcloud.kr/api/ApplyhomeInfoDetailSvc/v1/getRemndrLttotPblancDetail";// 요청 URL
	...
	public static List<RemainDetail> parsing(Date searchDate, int pageNum) {
		...
		try {
			...
			for(int i = 0; i< jsonArr.size(); i++) {
				...
				String gnrl_rcept_bgnde=getStrData(obj,"GNRL_RCEPT_BGNDE");  //특별공급접수시작일
				String gnrl_rcept_endde=getStrData(obj,"GNRL_RCEPT_ENDDE");  //특별공급접수종료일
				...
				String subscrpt_rcept_endde = getStrData(obj, "SUBSCRPT_RCEPT_ENDDE"); 					//청약접수종료일
				int tot_suply_hshldco = getIntData(obj, "TOT_SUPLY_HSHLDCO"); //공급규모
     
				
				RemainDetail rem = new RemainDetail(remainNo,   bsns_mby_nm,   cntrct_cncls_bgnde,   cntrct_cncls_endde,
						  gnrl_rcept_bgnde,   gnrl_rcept_endde,   hmpg_adres,  house_manage_no, house_dtl_secd_nm,
						  house_nm,   house_secd_nm,   hssply_adres,   hssply_zip,   mdhs_telno,
						  mvn_prearnge_ym,  pblanc_no, pblanc_url,   przwner_presnatn_de,   rcrit_pblanc_de,
						  spsply_rcept_bgnde,   spsply_rcept_endde,   subscrpt_rcept_bgnde,
						  subscrpt_rcept_endde, tot_suply_hshldco  );
				list.add(rem);
				
				System.out.println(rem.toString() + "삽입 완료!");
			}                                                             
